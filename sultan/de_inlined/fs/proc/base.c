...
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
		/* v2.1.0: Prevent memory access for hidden maps when unmounted */
...
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
	/* v2.1.0: Spoof readlink output for redirected paths (Stealth Mode) */
	...
	if (nr_files) {
		...
		for (i = 0, vma = mm->mmap, pos = 2; vma;
			...
			if (!vma->vm_file)
				continue;
#ifdef CONFIG_KSU_SUSFS_SUS_MAP
			/* v2.1.0: Skip entries for hidden memory maps in readdir */
